...
export interface groupByDatePara {
  folderDate: string
  folderName: string
}
export interface groupByDateReturn {
  date: string
...
export interface useFetchResponse<T> { data: T }
...
export function fetcher(...args: [RequestInfo, RequestInit?]) {
  return fetch(...args).then((res: Response) => res.json())
}
